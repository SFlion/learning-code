...
def DepreciationRateall(brand='dazhong'):
    try:
    ...
    for ye in range(2010,2015):
        ...
        and volume>1.9 and volume<3.1 and units>%d'''%(ye,brand,100)
        cursor.execute(sql)
        ...
        if y0[ye-2010]>0:
            ...
            for i in range(0,n):
                ...
                addv=addv+sqrt((d_temp[i]-(y0[ye-2010]+\
                (2.4-v_temp[i])/10-(abs(2.4-v_temp[i])/(2.4-v_temp[i]))*\
                (2013-ye)*0.01))**2)/(y0[ye-2010]+(2.4-v_temp[i])/10-\
                (abs(2.4-v_temp[i])/(2.4-v_temp[i]))*(2013-ye)*0.01)
    add0=add0/mark0
    addv=addv/mark0
    ...
    for ye in range(2010,2015):
        ...
        sql='''select depreciation_rate,volume from open_by_year_volume where year=%d
        and depreciation_rate is not null and depreciation_rate<1 and brand_slug='%s'
        and volume>1.9 and volume<2.1 and units>%d'''%(ye,brand,100)
        cursor.execute(sql)
        alldata=cursor.fetchall()
        cursor.close()
        n=len(alldata)
    ...
    for ye in range(2010,2015):
        ...
        sql='''select depreciation_rate,volume from open_by_year_volume where year=%d
        and depreciation_rate is not null and depreciation_rate<1 and brand_slug='%s'
        and volume>2.9 and volume<3.1 and units>%d'''%(ye,brand,100)
        cursor.execute(sql)
        alldata=cursor.fetchall()
        cursor.close()
        n=len(alldata)
    ...
    and volume>1.9 and volume<3.1 and units>%d'''%(brand,100)
    cursor.execute(sql)
    alldata=cursor.fetchall()
    cursor.close()
